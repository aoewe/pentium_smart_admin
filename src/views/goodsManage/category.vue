<template>
  <div v-loading="loading" element-loading-text="加载中" element-loading-spinner="el-icon-loading" class="main-box category">
      ...
      <el-table :data="list" row-key="id" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
        ...
        <el-table-column label="分类图片">
          <template slot-scope="scope">
            <el-image :src="scope.row.category_img">
        ...
        <el-table-column label="分类层级">
          <template slot-scope="scope">
            <span class="span">{{scope.row.category_level===1?'一':scope.row.category_level===2?'二':'三'}}级分类</span>
          </template>
        </el-table-column>
...
<script>
import {getCategotyList,getCategory,editCategory,changeStatus} from '@/api/goods'
  ...
  data() {
    return {
      ...
      rules:{
        ...
        category_status: [{ required: true, message: '请选择分类状态', trigger: 'blur' }],
