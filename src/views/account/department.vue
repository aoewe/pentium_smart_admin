<template>
  <div v-loading="loading" element-loading-text="加载中" element-loading-spinner="el-icon-loading" class="department main-box">
    <!-- <div class="select">
      <span>类型筛选</span>
    ...
    <el-button icon="el-icon-plus" type="primary" plain @click="dialogVisible = true, text = '添加部门'">添 加</el-button>
    ...
    <div class="list">
      <el-table :data="list" style="width: 100%">
        ...
        <el-table-column prop="name" label="部门" min-width="200" />
...
<script>
import { getDepartmentList,editDepartmentList } from '@/api/user'
export default {
  name: 'Department',
  data() {
    return {
  ...
  methods: {
    submit() {
      this.$refs['form'].validate(async value => {
        if (value) {
          ...
          const res = await editDepartmentList(params)
          if (res.code === 0) {
            this.$message({
              type: 'success',
              message: this.text === '添加部门' ? '添加成功' : '编辑成功'
            })
            this.initForm()
            this.getDepartmentList()
          }
    ...
    async getDepartmentList() {
      ...
      const { code, data } = await getDepartmentList(params)
      if (code === 0) {
        this.list = data.list
